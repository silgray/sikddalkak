...
- **`SelectionHandles.tsx`** — **모바일 선택 범위 양끝 드래그 핸들.** 홀드로 잡은
  ...
  **디버그**: `features.ts` 의 `RAW_CARET_DEBUG`(기본값 = 개발 서버에서만 켜짐)를
  켜면 파생 **전** 원시 캐럿 두 개가 가는 빨간 점선으로 겹쳐 그려지고
  (`.sel-raw-caret`), 드래그 중 콘솔에 두 줄이 찍힌다: `[rawCaret]` 은 **오프셋
  판정**(`tapOff` = 그 자리를 탭했을 때 값, `off` = 우리가 쓴 값 — 지금은 같아야
  한다), `[rawCaret:draw]` 는 **그리는 좌표**(`trueX` vs `drawX`). 둘을 갈라
  찍는 이유는 "값이 틀린 건지 그리는 게 틀린 건지" 를 가리려는 것이다 —
  ⚠ 그리는 좌표는 `applyDragAt` 에서 못 찍는다(아직 렌더 전이라 늘 한 스텝 낡은
  값이 나온다, 실측). 그래서 `measure()` 안에서 찍는다. 손가락이 짚은 자리와 파랗게 칠해진 범위는 서로 다를 수
