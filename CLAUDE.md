...
- **`mf.getOffsetFromPoint(x, y, {bias})` 는 공개 API다** — 화면 좌표 → 모델 오프셋.
  ...
  ⚠ **그런데 이 히트테스트는 `first` 센티넬로 튄다.** 센티넬 원자의 상자를 재면
  ...
  `MathField` 렌더는 걸린다). 그래서 이 API는 날로 쓰지 말고 **`internals.ts` 의
  `resolveOffsetAt`** 을 쓴다 — 센티넬이 나오면 그 branch 안에서 손가락에 가장
  가까운 형제 경계로 고쳐준다.
...
- **`SelectionHandles.tsx`** — **모바일 선택 범위 양끝 드래그 핸들.** 홀드로 잡은
  ...
  **스냅을 자기가 계산하지 않는다** — 선택을 그냥 세팅하면 `MathField` 의
  ...
  준다(왼쪽 끝=-1, 오른쪽 끝=+1) — 0이면 원자 한가운데가 기준이라 얹은 원자가
  빠진다(실측). ⚠ 그 방향을 **잡은 핸들의 정체로 정하면 안 된다** — 교차하는 그
  이동에서는 이미 낡은 값이다. bias 0으로 한 번 가늠해 어느 쪽이 될지 정한 뒤
  그 방향으로 다시 잰다(두 번째 호출은 `atomBoundsCache` 가 더워 싸다).
  **드래그는 `editor/rawSelection.ts` 의 원시 캐럿을 옮긴다** — 스냅된 선택 자체를
  옮기면, 홀드 드래그가 구조 경계를 넘어 상위로 스냅된 뒤엔 되돌릴 방법이 없다
  (스냅된 결과가 다음 조작의 출발점이 되어버리므로).
  **핸들은 반대쪽 캐럿을 넘어갈 수 있다** — 넘어간 쪽이 새 시작이 된다. 막는 건
  교차가 아니라 **겹침**뿐이다(두 캐럿이 같은 자리면 선택할 게 없다): 그때만 가던
  ...
  (손잡이 위/아래·핀 화살촉 방향)를 갈라 놓는다 — 안 그러면 교차하는 순간 쥔
  핸들이 반대편으로 순간이동한다. 컨테이너 밖으로 나간 끝은
...
## ⚠ 이 브랜치(`mobile-kbd-palette`)의 대원칙 — 모바일에만 적용한다

**이 브랜치에서 하는 변경은 가능한 한 전부 모바일 환경에서만 적용되어야 한다.**
데스크톱 동작·레이아웃은 건드리지 않는다. 이 브랜치는 모바일 입력 수단 실험용이고,
여기서 또 다른 실험 브랜치들이 갈라져 나가므로 데스크톱이 흔들리면 비교가 무너진다.
