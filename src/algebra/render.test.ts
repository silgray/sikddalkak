...
describe('괄호는 필요한 곳에만', () => {
  ...
  it('곱의 묶음은 왼쪽으로 정규화된다', () => {
    // 행렬곱은 결합법칙이 있으므로 `A(BC)` 와 `(AB)C` 는 같은 식이다. 묶음을 한쪽으로
    // 모아두면 렌더가 안정된다. **순서**는 그대로다 — `AB ≠ BA` 는 여전히 지켜진다.
    expect(rendered(String.raw`A\left(BC\right)`)).toBe('ABC');
    expect(rendered(String.raw`\left(AB\right)C`)).toBe('ABC');
  ...
  it('내적 뒤의 곱은 스칼라곱이라 내적 쪽에 괄호가 붙는다', () => {
    // `u·v` 가 스칼라이므로 그 다음 곱은 내적이 아니라 스칼라곱이다.
    expect(sexpTyped(typedOf(String.raw`u\cdot v\cdot w`))).toBe('(scalarMul (dot u v) w)');
