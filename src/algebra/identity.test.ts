...
describe('모양 해석 — elaborate가 I 를 채운다', () => {
  it('A+I 는 I 를 A의 모양으로 해석한다', () => {
    ...
    expect(formatTyped(t)).toBe('(add I A)');
    expect(formatShape(t.shape)).toBe('3x3');
  });

  it('AI, IA 는 상대에게서 I의 모양을 유도해 소거한다', () => {
    expect(formatTyped(normalizedOf('AI'))).toBe('A');
    expect(formatTyped(normalizedOf('IA'))).toBe('A');
  });

  it('직사각·벡터와도 소거된다', () => {
    expect(formatTyped(normalizedOf('Iv'))).toBe('v'); // v: 3x1
    expect(formatTyped(normalizedOf('MI'))).toBe('M'); // M: 2x3 직사각
  });

  it('AIA 는 I 를 먼저 버려야 두 A 가 이웃해져 A² 로 접힌다', () => {
    // 소거와 접기가 둘 다 normalize 몫이다 — parse 만으로 여기까지 온다.
    const t = normalizedOf('AIA');
    expect(formatTyped(t)).toBe('(matPow A 2)');
    expect(render(t)).toBe('A^{2}');
  });

  it('A^0 은 I 다', () => {
    const t = normalizedOf('A^0');
    expect(formatTyped(t)).toBe('I');
    expect(formatShape(t.shape)).toBe('3x3');
  });

  it('I^n 은 미정이어도 정사각이 전제이므로 그대로 I 다', () => {
    expect(formatTyped(normalizedOf('I^3'))).toBe('I');
  });
});
...
describe('거듭제곱 접기 — normalize 기본값', () => {
  ...
  it('지수 합이 0이면 소거된다', () => {
    expect(formatTyped(normalizedOf(String.raw`AA^{-1}`))).toBe('I');
...
describe('matPow 지수는 식이어도 된다', () => {
  it('A^{1+2} 는 A^3 이다 — 지수도 정규화된다', () => {
    ...
    expect(formatTyped(t)).toBe('(matPow A 3)');
  ...
  it('직접 쓴 정수 지수의 모습은 그대로다', () => {
    ...
    expect(formatTyped(normalizedOf(String.raw`A^{-1}`))).toBe('(matPow A -1)');
    expect(formatTyped(normalizedOf('A^{2}'))).toBe('(matPow A 2)');
  });

  it('값이 안 정해진 지수도 통과한다 (치환 뒤에 정해질 수 있다)', () => {
    const t = normalizedOf('A^{k}');
    expect(formatTyped(t)).toBe('(matPow A k)');
    // 렌더 왕복이 성립해야 한다.
...
describe('역행렬 소거 — 퍼즈가 못 보는 자리, 표 테스트로만 검증', () => {
  it('A^{-1}A 는 I 다', () => {
    expect(formatTyped(simplifiedOf(String.raw`A^{-1}A`))).toBe('I');
  });

  it('AA^{-1}B 는 B 다 (소거로 새로 생긴 I 를 다시 걷어낸다)', () => {
    expect(formatTyped(simplifiedOf(String.raw`AA^{-1}B`))).toBe('B');
  });

  it('BA^{-1}A 도 대칭적으로 성립한다', () => {
    expect(formatTyped(simplifiedOf(String.raw`BA^{-1}A`))).toBe('B');
  });
});
...
describe('env에 I 를 직접 정의하면 사용자 정의가 이긴다', () => {
  it('보통 심볼처럼 동작한다 — 자동 소거가 꺼진다', () => {
    ...
    expect(formatTyped(t)).toBe('(matMul A I)'); // matIdentity 가 아니라 평범한 matMul
...
describe('값 대조 — 소거가 값을 바꾸지 않는다', () => {
  ...
  for (const latex of cases) {
    it(`${latex}`, () => {
      ...
      if (latex === 'A^0') {
        expect(formatTyped(eliminated)).toBe('I');
        return;
      }
